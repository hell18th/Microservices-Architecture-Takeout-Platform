...
public class UserAuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            // 将ID添加到请求头中
            ServerWebExchange webExchange = exchange.mutate().request(builder -> builder.header(JwtClaimsConstant.USER_ID, String.valueOf(id))).build();
    ...
    private boolean isExcludePath(String path) {
        if (!antPathMatcher.match(authProperties.getUserPath(), path)) {
            return true;
        }
        for (String excludePath : authProperties.getUserExcludePaths()) {
